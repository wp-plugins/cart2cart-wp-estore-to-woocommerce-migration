jQuery(document).ready(function($) {
  ...
  function installBridge(install, token){
    var url = SELF_PATH + 'task=' + install + 'Bridge&format=raw';
    if (token){
      url=url+'&token='+token;
    }
    console.log("try to install-bridge");
  ...
  function installRemoteBridge(){
    var email =cart2CartLoginEmail.val();
    var pass = cart2CartLoginKey.val();
    console.log("try to install-bridge");
    $.ajax({
      cache: false,
      dataType: 'jsonp',
      jsonpCallback: 'callback',
      url: HTTP_API_PATH + 'install-bridge',
      data: {
        'email': email,
        'pass': pass,
        'ftpHost': Cart2cartRemoteHost.val(),
        'ftpUser': Cart2cartRemoteUsername.val(),
        'ftpPass': Cart2cartRemotePassword.val(),
        'ftpDir': Cart2cartRemoteDirectory.val()
      },
      success: function(data){
        if (data.error == false){
          errorMessage('Connection Bridge installed','success');
